...
5. 使用者已明確同意重啟後，執行已安裝套件的 `apply-dahye-skin.ps1`。它會由 WMI 建立不屬於 Codex 的獨立 worker，再由 worker 執行 `start-dahye-skin.ps1 -RestartExisting`，關閉目前 Codex 並以官方 Store 執行檔與本機回環 CDP 重新啟動。
6. 重啟後確認 `runtime\apply-result.json` 通過，再執行 `verify-dahye-skin.ps1 -ScreenshotPath <絕對路徑>`，檢查首頁、一般任務頁、原生側欄、四張原生卡片、專案選擇器、輸入框，以及亮色與深色模式。
